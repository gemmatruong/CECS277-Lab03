...
def get_char(prompt):
    """Repeatedly takes in and validates user's input to ensure that it is a single alphabetic character.
    Args:
        prompt: string to display to the user to prompt them to enter an input.
    Returns:
        The valid single alphabetic character input in uppercase.
    """
    valid = False
    while not valid:
        val = input(prompt).upper()
